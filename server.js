...
function armDrawTimer(room){
  ...
  if(!room.started || room.pendingDiscard || !current || current.hasDrawn){ room.turnDeadline=null; room.turnPhase=null; return; }
  room.turnPhase='draw';
  room.turnDeadline=Date.now()+DRAW_MS;
  ...
  room.turnTimer=setTimeout(()=>{
    ...
    room.turnDeadline=null; room.turnPhase=null;
    addLog(room,`⏱️ ${p.name} 10秒内未摸牌，自动跳过本轮`);
...
const TILE_COUNTS = {
  ...
  '老':3,'板':3,'店':2,'长':2,'前':2,'现':2,'任':2,'陪':3,'玩':3,'哥':2,'姐':2,'客':1,'人':1,'主':1,'友':1,'鸡':2,'鸭':2,'狗':2,'宝':2,'贝':2,'富':1,'婆':1,
  '游':1,'戏':1,'局':2,'排':1,'位':1,'分':1,'段':1,'上':2,'下':1,'输':1,'赢':1,'开':1,'黑':2,'组':1,'队':1,'野':1,'王':1,'者':1,'号':1,'服':1,'区':1,'麦':2,'语':1,'音':1,'房':2,'间':1,'单':3,'双':1,'坑':2,'带':2,
  '接':2,'点':2,'报':2,'警':2,'包':1,'时':1,'钱':3,'付':1,'款':1,'退':2,'续':1,'加':2,'价':1,'免':1,'费':1,'刷':2,'榜':1,
...
function startRound(room, isRestart=false){
  ...
  if(isRestart){
    for(const entry of room.scoreboard.values()) entry.totalScore=0;
  }
  for(let r=0;r<13;r++) for(const p of room.players) p.hand.push(room.deck.pop());
  addLog(room, isRestart ? '🔄 房主已重开本局：所有玩家留在房内，重新发牌，房间积分全部清零并从0重新开始。' : '游戏开始！每人13张；轮到自己后10秒内摸牌，摸牌后开始45秒出牌倒计时。累计达到30分判定获胜。');
