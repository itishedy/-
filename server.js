...
function resolvePendingDiscard(room, approved){
  ...
  if(approved){
    ...
    p.hasDrawn=true;
    addLog(room,`❌ 「${pd.tiles.join('')}」未获过半认可，牌已退回 ${p.name}，请重新出牌`);
  }
  broadcast(room);
}
...
io.on('connection', socket=>{
  ...
  socket.on('startGame', ()=>{
    ...
    room.deck=buildDeck(); room.discardGroups=[]; room.pendingDiscard=null; room.turn=0; room.started=true; room.lastWin=null;
    room.players.forEach(p=>{p.hand=[];p.hasDrawn=false;});
    for(let r=0;r<13;r++) for(const p of room.players) p.hand.push(room.deck.pop());
    addLog(room,'游戏开始！每人13张；轮到你先摸到14张，再打出至少3个字，其他玩家投票过半才算出牌成功。'); broadcast(room);
  });
  socket.on('draw', ()=>{
    ...
    if(p.hasDrawn) return socket.emit('errorMsg','你已经摸过牌了');
  ...
  socket.on('discard', ({indices})=>{
    ...
    const clean=[...new Set(indices.map(Number))];
    if(clean.length<3) return socket.emit('errorMsg','每次出牌至少需要3个字');
    if(clean.some(i=>!Number.isInteger(i)||i<0||i>=p.hand.length)) return socket.emit('errorMsg','选中的牌无效');
  ...
  socket.on('declareWin', ({sentence})=>{
    ...
    if(p.hand.length!==14) return socket.emit('errorMsg','必须在摸牌后的14张牌时胡牌');
